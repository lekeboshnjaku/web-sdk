...
export const bookEventHandlerMap: BookEventHandlerMap<BookEvent, BookEventContext> = {
	...
	createBonusSnapshot: async (bookEvent: BookEventOfType<'createBonusSnapshot'>) => {
		...
		// Restore bonus UI silently on resume — do not replay the full FS intro
		// (intro waits for a click and looks frozen after refresh).
		if (lastFreeSpinTriggerEvent) {
			...
			await eventEmitter.broadcastAsync({
				type: 'globalMultiplierUpdate',
				multiplier: lastUpdateGlobalMultEvent?.globalMult ?? 1,
			});
			eventEmitter.broadcast({ type: 'freeSpinCounterShow' });
			eventEmitter.broadcast({
				type: 'freeSpinCounterUpdate',
				current: lastUpdateFreeSpinEvent?.amount,
				total: lastUpdateFreeSpinEvent?.total ?? lastFreeSpinTriggerEvent.totalFs,
			});
			await eventEmitter.broadcastAsync({ type: 'uiShow' });
			await eventEmitter.broadcastAsync({ type: 'drawerButtonShow' });
			eventEmitter.broadcast({ type: 'drawerFold' });
		...
		if (lastSetTotalWinEvent) {
			stateBet.winBookEventAmount = lastSetTotalWinEvent.amount;
		}
	},
};